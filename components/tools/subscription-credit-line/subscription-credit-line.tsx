...
export function SubscriptionCreditLine() {
  ...
  const getInitialInput = (): SubscriptionLineInput => {
    ...
    return {
      ...
      deploymentPaceType: (searchParams.get('deployPace') as 'front-loaded' | 'linear' | 'back-loaded') || defaultInput.deploymentPaceType,
      managementFeeRate: parseFloat(searchParams.get('mgmtFee') || '') || defaultInput.managementFeeRate,
      managementFeeBasis: (searchParams.get('feeBasis') as 'commitments' | 'invested-capital') || defaultInput.managementFeeBasis,
